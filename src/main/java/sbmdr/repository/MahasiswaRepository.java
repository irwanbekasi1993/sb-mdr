...
public interface MahasiswaRepository extends JpaRepository<Mahasiswa, String> {
	...
	Mahasiswa findByNamaMahasiswaAndEmail(String username, String email);

	@Modifying
	@Query(value = "insert into mahasiswa (nim,nama_mahasiswa,email,status_mahasiswa) "
			+ "values(:nim, :namaMahasiswa,:email,:statusMahasiswa)", nativeQuery = true)
	@Transactional
	int insertDataMahasiswa(
			@Param("nim")String nim,
			@Param("namaMahasiswa")String namaMahasiswa, 
			 @Param("email")String email,
			 @Param("statusMahasiswa")String statusMahasiswa);
